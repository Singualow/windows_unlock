...
- 支持手动修改解锁/锁定 RSSI 阈值、距离校准、自动锁定、高灵敏响应、认证失败冷却开关、暂停、撤销手机、更新 Windows 密码和完整卸载。
...
- 高灵敏模式使用独立的近距离触发阈值，并自动在其下方保留 `8 dB` 锁定防抖区。
  锁屏时首个符合阈值的新鲜广播会在约 `0.2` 秒内发起认证；失去有效证明约 4 秒
  后锁定，持续低于派生锁定线约 2 秒后锁定。该模式响应更快，但蓝牙短暂波动可能
  误锁，并会增加手机耗电。
- “设备”页可直接修改普通模式的解锁/锁定阈值，以及高灵敏模式的独立触发阈值。
  普通解锁和锁定阈值必须至少相差 `8 dB`；高灵敏模式会自动派生低 `8 dB` 的
  锁定保护线，避免在边界反复锁定。
